// Popup logic for displaying and submitting job data
...
document.addEventListener('DOMContentLoaded', async () => {
  // Try to upgrade to side panel if supported (Chrome 114+)
  if (chrome.sidePanel && typeof chrome.sidePanel.open === 'function') {
    try {
      ...
      // Side panel not supported or failed, continue with popup
      console.error('Failed to open side panel, using popup:', error);
    }
  }

  // Continue with popup (fallback for browsers without side panel support)
  // Get current tab URL
...
/**
 * Detect job data from current page
 */
async function detectJobData() {
  try {
    ...
    chrome.tabs.sendMessage(tab.id, { type: 'DETECT_JOB_DATA' }, (response) => {
...
function setupDraftHandlers() {
  ...
  const openTabButton = document.getElementById('openInTabButton');
  ...
  if (openTabButton) {
    openTabButton.addEventListener('click', async () => {
      // Save current draft before opening tab
      await saveCurrentDraft();

      // Open form in new tab
      chrome.tabs.create({
        url: `src/form/form.html?url=${encodeURIComponent(currentUrl)}`
      });
